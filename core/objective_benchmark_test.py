...
if __name__ == "__main__":
    ...
    zone = "objective_benchmark_test"
    current_dir = os.path.dirname(os.path.abspath(__file__))
    source_file_path = os.path.join(current_dir, zone + ".json")

    data = load_questions_from_file(source_file_path)
    num = len(data)
    prompt ="请你做一道自然科学相关的题目。\n请你一步一步思考。你将从A，B，C，D中选出正确的答案，并写在【答案】和<eoa>之间。\n例如：【答案】: A <eoa>\n完整的题目回答的格式如下：\n【解析】 ... <eoe>\n【答案】 ... <eoa>\n请你严格按照上述格式作答,请你严格按照上述格式作答,请你严格按照上述格式作答。\n题目如下："

    model_answer_dictlist = to_tackle_questions(data,0,num,DeepSeek_R1,prompt)

    write_results_to_file(zone,prompt,DeepSeek_R1,model_answer_dictlist)
